...
def userFromNumber(connection, from_number):
    ...
    if len(result) == 1:
        ...
        msu_id = result[0][6]
        message = result[0][7]
    return receiver_id, related_msg_id, channel, receiver_address_as, waiting, sender, project_name, msu_id, message

def set_whatsapp_last_received(connection, user_id):
    update_sql = 'UPDATE users SET whatsapp_last_received = NOW() WHERE id = ' + str(user_id)
    print(update_sql)
    connection.run(update_sql)


def incomingMessage(connection, user_id, related_msg_id, message, channel):
    insert_sql = "INSERT INTO msgs_received (message,user_id,related_msg_id,channel) VALUES (" + literal(message) + "," + str(user_id) + ","+ str(related_msg_id) + ", " + literal(channel) + ")"
    print(insert_sql)
...
def update_message(address_as, sender, project_name, message, phone, channel):
    ...
    intro_message = "Hello " + address_as + ",\n" + sender + " wanted to share the latest update to the " + project_name + " program.\n\n" 
...
def update_log_msg_sent_user(connection, msu_id, success, channel, waiting, declined=None):
    if declined is None:
        declined_string = 'NULL'
    else:
        declined_string = str(declined)
...
def outgoingMessage(connection, user_id, sender, prj_id, message, project_name, related_item=None):
    sendTwilio = setupTwilio()
    insert_sql = "INSERT INTO msgs_sent (prj_id, message, user_id_sending, related_item) VALUES (" + str(prj_id)+ "," + literal(message) + "," + str(user_id) + ","
    insert_sql += "NULL" if related_item is None else literal(related_item)
    insert_sql += ") RETURNING id"
    print(insert_sql)
    msg_id = connection.run(insert_sql)[0][0]

    sql = 'SELECT id, address_as, phone, in_window FROM users_w_numbers WHERE prj_id = ' + literal(prj_id)
    ...
    for result in results:
        ...
        if in_window:
            success = update_message(address_as, sender, project_name, message, phone, channel)
            waiting = False
            declined = False
        else:
            ...
            waiting = True
            declined = None
        log_msg_sent_user(connection, user_id, msg_id, success, channel, waiting, declined)
...
def getMessages(connection, prj_id, related_item, since_sent_id, since_received_id):
    sql = '''SELECT s.id, s.sent_time, s.user_id_sending, s.message, r.id, r.received_time, r.user_id, r.message
                ...
                WHERE s.prj_id = ''' + str(prj_id) 
    sql +=  '   AND s.related_item = ''' + literal(related_item)
    sql +=  '   AND (s.id > ' + str(since_sent_id) + ' OR r.id > ' + str(since_received_id) + ') '
...
def lambda_handler(event, context):
    ...
    if method == 'GET':
        ...
        since_received_id = int(params['since_received_id']) if 'since_received_id' in params else 0
        result = getMessages(connection, prj_id, related_item, since_sent_id, since_received_id)
        ...
        try:
            ...
            user_id, related_msg_id, channel, address_as, waiting, sender, project_name, msu_id, outgoing_message = userFromNumber(connection, phone_number)
            message = params['Body']
            ...
            if waiting and first_word == 'no':
                update_log_msg_sent_user(connection, msu_id, success=False, channel=channel, waiting=False, declined=True)
            else:
                if channel == 'w':
                    set_whatsapp_last_received(connection,user_id)
                if waiting: # (and first_word != 'no', which is implied)
                    success = update_message(address_as, sender, project_name, outgoing_message, phone_number, channel)
                    waiting = not success
                    update_log_msg_sent_user(connection, msu_id, success, channel, waiting, declined=False)
            ...
            outgoingMessage(connection, user_id, sender, prj_id, message, project_name, related_item) 
