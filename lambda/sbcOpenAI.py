...
FORMATS = {"list":"Respond only with a list and without any other text.",
...
def call_openai_api(prompt, context, stop):
    ...
    openai.api_key = secret["Authorization"]
    retries = 5
    backoff = 1
...
def lambda_handler(event, context):
    ...
    if allowance_ok(prj_id):
        ...
        s3.put_object(Bucket=BUCKET_NAME, Key=f"{request_id}.json", Body=json.dumps(response), ContentType='application/json')
