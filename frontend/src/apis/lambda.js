...
export async function downloadObjects(objects, context, tablePrefix='', LOG=false) {
    for (let objName in objects) {
        if (LOG) {
            console.log('preparing to download object:'+objName+ ' prj_id='+useProjectStore().prj_id);
        }
...
export async function downloadDictionary(tableName, attributes, filterClause = '', keyIndex) {
    const response = await downloadSetup(tableName, attributes, filterClause);
...
export async function gptCompletion(prompt, context = null, format = null, start = null, stop = null) {
    ...
    if (context) {
      console.log("Words in GPT context + prompt: ", countWords(prompt) + countWords(context));
      payload.context = context;
    }

    let response;
    if (true) {  // set to false when testing other features to avoid unnecessary calls to openai
      try {
        response = await axios.post(SBC_AI_URL, payload, {
          headers: {
