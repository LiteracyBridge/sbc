...
export const useProjectStore = defineStore({
  ...
  getters: {
    projects: (state) => (archived = false) => state.user_projects.filter((p) => p.archived == archived),
    userById: (state) => (userId) => state.users_in_project.find((u) => u.user_id == userId),
    ...
    projectName: (state) => {
      if (state.prj_id && state.user_projects.length) {
        const foundProject = state.user_projects.find((p) => p.prj_id == state.prj_id);
        if (foundProject) {
    ...
    grantableAccess: (state) => {
      if (state.prj_id && state.user_projects.length) {
        const user_access = state.user_projects.find((p) => p.prj_id == state.prj_id).access_id;
        const grantable_access_types = useLookupStore().access_types.filter((a) => a.id >= user_access);
  ...
  actions: {
    ...
    async addUser(name, email, access_id, address_as = '') {
      ...
      const id = await api.insert('project_users', { prj_id, user_id, access_id });
    ...
    async add(name, country_id) {
      ...
      const prj_id = await api.insert('projects', { name, country_id, private_prj });
    ...
    async download() {
      if (this.prj_id) {
        let response = await api.downloadObject('users_in_project', ['id', 'user_id', 'email', 'name', 'access_id', 'address_as'], 'prj_id=' + this.prj_id, true);
        this.users_in_project = response;
      }
      if (useUserStore().id) {
        let response = await api.downloadObject(
        'user_projects',
        ['prj_id', 'name', 'private_prj', 'country_id', 'access_id', 'archived'],
