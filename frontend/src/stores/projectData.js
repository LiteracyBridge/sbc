...
export const useProjectDataStore = defineStore({
  ...
  state: () => ({
    questions: [
      {id:0,topic:"basic",topic_id:0,bulb:true,
      q2u:"What is the name of your project?", 
      q2ai:"Propose a name for this SBC program.",
      f4ai:"item",label:"Program Name"
      }, 
      {id:1,topic:"basic",topic_id:1,bulb:true,
      q2u:"What sector is this project working in?", 
      q2ai:"Propose a sector for this project.",
      f4ai:"item",label:"Sector"
      },
      {id:2,topic:"basic",topic_id:2,bulb:false,
      q2u:"Where is your project working?", 
      q2ai:"",
      f4ai:"item",label:"Location"
      },
      {id:3,topic:"basic",topic_id:3,bulb:false,
      q2u:"What partner organizations are working on this project?", 
      q2ai:"",
      f4ai:"list",label:"Partners"
      },
      {id:4,topic:"basic",topic_id:4,bulb:true,
      q2u:"Write a summary paragraph for this project.", 
      q2ai:"Summarize this project in one paragraph.",
      f4ai:"item",label:"Project Summary"
      },
      {id:5,topic:"background",topic_id:0,bulb:false,
      q2u:"What challenge is your project working to address?", 
      q2ai:"",
      f4ai:"item",label:"Challenges"
      },
      {id:6,topic:"background",topic_id:1,bulb:false,
      q2u:"What are the root causes of that challenge?", 
      q2ai:"",
      f4ai:"list",label:"Root Causes"
      },
      {id:7,topic:"background",topic_id:2,bulb:false,
      q2u:"What other background information do you have on the context of your project? (social, economic, cultural, political, etc.)", 
      q2ai:"",
      f4ai:"item",label:"Other background"
      },
      {id:8,topic:"objectives",topic_id:0,bulb:false,
      q2u:"What is your project’s vision? What would you like to see at the end of your project?", 
      q2ai:"",
      f4ai:"item",label:"Project Vision"
      },
      {id:9,topic:"objectives",topic_id:1,bulb:true,
      ...
      f4ai:"list",label:"Targeted Behaviors"
      },
      {id:10,topic:"objectives",topic_id:2,bulb:true,
      ...
      f4ai:"list",label:"Objectives"
      },
      {id:11,topic:"audiences",topic_id:0,bulb:true,
      ...
      f4ai:"item",label:"Primary Audience"
      },
      {id:12,topic:"audiences",topic_id:1,bulb:true,
      ...
      f4ai:"list",label:"Influencers"
      },
      {id:13,topic:"audiences",topic_id:2,bulb:true,
    ...
    topics:[
      ...
      {id:4,sequence:5,value:"approaches",label:"Approaches and Activities"},
      // {id:1,sequence:6,value:"toc",label:"Theory of Change"},
      {id:5,sequence:7,value:"communications",label:"Communications and Messaging"},      
      {id:6,sequence:8,value:"monitoring",label:"Monitoring and Evaluation"},
      {id:7,sequence:9,value:"prjmgmt",label:"Project Management"},
      {id:8,sequence:9,value:"prjdocs",label:"Project Documents"},
    ]
  }),
  getters: {
    questionsForTopic:(state) =>
      (topic) => state.questions.filter((q)=>q.topic==topic), 
    countBulbs:(state) =>
      (id,topic) => state.questions.filter(question => 
        (question.id <= id && question.bulb === true && question.topic==topic)).length,
    answerForQuestionId:(state) =>
      (questionId) => state.project_data.filter((d)=>d.q_id==questionId)[0],
    getData:(state) =>
      (questionId) => state.project_data[questionId] ? state.project_data[questionId].data : '',      
  }, 
  actions: {
    clear() {
  ...
  async download() {
    ...
    this.project_data = await api.downloadDictionary('project_data',['id', 'q_id', 'editing_user_id', 'data'],filter_clause,key_index);
},
  ...
  async setData (q_id,data) {
    if (!(q_id in this.project_data)) {
      ...
      if (id) {
        ...
        this.project_data[q_id].id = await api.insert('project_data', {q_id, data}); 
      }
    },
