  ...
  getters: {
    ...
    factorsByCategory: (state) => (category_id: number, filter: string) => {
      if (filter == "project") {
        ...
        return state.lu_drivers.filter(
          (d) => d.category_id === category_id && d.parent_id === 0
        );
      } else {
        // filter == 'suggested'
        return state.lu_drivers.filter(
    ...
    driversInProject: (state) => {
      ...
      for (const driver of state.drivers_in_prj) {
        const luDriver = state.lu_drivers.find(
          (d) => d.id == driver.lu_driver_id
        );
        const id = driver.id;
  ...
  actions: {
    ...
    async suggestDrivers() {
      ...
      for (let i = 0; i < category_list.length; i++) {
        ...
        question +=
          category_list[i].num_suggestions +
          " driver(s) from the " +
          category_list[i].name +
          " category";
      }
      question +=
        " that would be most influential to achieving the SBC objectives of this project.";
      question +=
        "In addition, include an EXPLANATION (2-4 sentences) with specific reasons why elements of this project align to your selected dimension.";
      question +=
        "Finally, include an EXAMPLE (3-5 sentences) describing how this driver was found to influence SBC in an actual project. Focus on the influences of this driver before the project interventions began.";
      question +=
      ...
      question +=
        "Provide an EXPLANATION (2-4 sentences) with specific reasons why elements of this project align to your selected dimension.";
      question +=
        "Finally, include an EXAMPLE (3-5 sentences) describing how this dimension was found to influence SBC in an actual project. Focus on the influences of this dimension before the project interventions began.";
      question +=
        "Give your answer as an array of json objects according to this format:\n";
      question +=
        '[{"id":0,"name":"","explanation":"","example":""}]\n\n(In place of the 0 for id, use the number preceding each dimension name)\n\n';
